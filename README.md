...
![截图](docs/screenshot-20260924.png)
...
> ⚠️ **重要声明：本仓库的打包/发布版本未经测试。**
> 我只在自己的机器（Ubuntu 24.04 + GNOME/Wayland + Waydroid）上跑通，打包出来的压缩包
> **没有在干净环境验证过**，可能缺依赖、路径不通或直接报错。请把它当作「能调的半成品」，
> 按下面「依赖 / 安装」自己装好 `maa-cli`、`adb` 等再按需修改。
...
- **三列布局（对齐官方 TaskQueueView）**：左＝任务卡（勾选/拖拽排序/⚙），中＝逐任务设置，右＝运行日志。
- **实时日志 + 任务状态**：解析 `maa -v` 输出，任务状态/耗时显示在左侧任务卡上（运行中/完成/失败配色）。
- **静默 / 看着跑**：「显示游戏窗口」勾选——勾上＝运行前打开 Waydroid 窗口（且立即打开）、不勾＝静默后台跑；
  **「监工(scrcpy)」** 开关可开一个可操作的小窗镜像（再点关掉，关掉不影响任务）。
...
- **说明一律用 ⓘ**（悬停展开），对应官方的 `TooltipBlock`。
- 详细的「每个任务有哪些设置/标题/选项」见 [`docs/官方设置项盘点.md`](docs/官方设置项盘点.md)。
...
- Release 里提供 `yuMAArin-<版本>.tar.gz`（就是本仓库文件打包）：**解压 → 看 README → `./install.sh`**。
- **再次强调：这个包没测过**，装完可能需要你自己调。
...
docs/screenshot-20260924.png    截图
docs/官方设置项盘点.md            每个任务的设置标题/选项/默认值（对照官方）
```
...
- ⚠ **「监工(scrcpy)」注意**：scrcpy 的解码压在核显上，**别频繁反复开关**；本机曾出现
  Intel 核显挂死（`i915 GPU HANG`）导致整机卡死。已降负载（720p/15fps/2Mbps/无音频），
  长时间「看着跑」更推荐用「显示游戏窗口」（Waydroid 原生窗口，少一路解码）。
